...
def create_immigration_fact_table(spark, df, output_data):
    ...
    df = df.withColumnRenamed('ccid', 'record_id') \
        .withColumnRenamed('i94res', 'country_residence_cd') \
        .withColumnRenamed('i94addr', 'state_cd')
...
#####cleaning functions#######################
def clean_immigration_data(df, spark=False):
    ...
    if spark==False:
        #pandas flavor
        # Dropping columns EDA has shown to have a high percentage of missing values
...
def clean_demographics_data(df, spark=False):
    ...
    if spark==False:
        ...
        print('{:,} rows prior to cleaning'.format(len(df)))
        ...
        print('{:,} rows after dropping NA values'.format(len(df)))
        
        # drop duplicate columns
        df = df.drop_duplicates(subset=['City', 'State', 'State Code', 'Race'])
        print('{:,} rows after dropping duplicate columns'.format(len(df)))
    else:
        #spark flavor
        # drop rows with missing values
        print('{:,} rows prior to cleaning'.format(df.count())
        ...
        #drop NA values
        df = df.dropna(subset=subset_cols)
        print('{:,} rows after dropping NA values'.format(df.count())

        #dropping duplicate columns
        df = df.dropDuplicates(subset=['City', 'State', 'State Code', 'Race'])
        print('{:,} rows after dropping duplicate columns'.format(df.count()))
    return df
####################################### 
...
def quality_checks(df, table_name):
    ...
    if total_count == 0:
        print(f"FAIL--{table_name} has zero records!")
    else:
        print(f"PASSED--{table_name} with {total_count:,} records.")
    return 0